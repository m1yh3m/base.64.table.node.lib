...
function validIndexRange () {
  return [i => i >= 0, i => i <= 63]
}

function isInValidIndexRange (index) {
  return validIndexRange().every(fn => fn(index))
}

function binaryForChar (ch) {
  const possibleIndex = indexForChar(ch)
  return isInValidIndexRange(possibleIndex) ? table[possibleIndex].binary : -1
}

function charForIndex (index) {
  return isInValidIndexRange(index) ? table[index].char : ''
}

function charForBinary (bin) {
  return containsBinary(bin) ? table.filter(i => i.binary === bin).pop().char : ''
}
...
function containsBinary (bin) {
  return table.some(i => i.binary === bin)
}
